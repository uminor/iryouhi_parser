...
#     e-taxの申告時に添付するのは不適切。
#
# （２）pdf には「医療費控除の明細書」に記載が必要な、
#　　　医療費などの「年度合計」が表示されていないので、別途合算が必要。
#     そもそも、少なくとも11月～12月受診分のpdf発行が申告時期に
#     間に合わないので、pdfの情報だけでは正確な年度合計が計算できない。
#
# （３）仮に pdf が証票として使えるとしても、e-tax で申告した場合
#    「別途郵送」の手間が発生する。
#
# そこで、本プログラムでは、
#
...
#    の該当部分にコピペする。
#    pdf に反映していなかった件があれば、この時点で手動で追加する。
#      (例：11月～12月の受診分など)
#
