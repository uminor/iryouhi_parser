...
# 確定申告すれば医療費控除が可能。
#
# しかしS社健保組合の「健康保険医療費のお知らせ」の pdf は
# 健保組合が認証した「原本」ではないので、
# 医療費控除の正式なエビデンスとしては使えなさそう。
# (＝本プログラム開発者の見解)
# ただ pdf 記載の情報は申告のための集計の助けにはなりそう。
#
# そこで本プログラムは、pdf 約1年分を読み込んで、
# 集計したExcelファイルを出力する。
...
# ≪注意事項≫ +++++++++++++++++++++++++++++++++++++++++++++++++++++
# 本プログラムの動作は無保証です。
# 入力データ依存して正しく動作しない可能性もありまる。
# 正しい申告データが作成されるかどうかも無保証です。
# （動作レポート・バグ報告は歓迎します。）
#
# 参考として、上述の通り、S社健保組合の「健康保険医療費のお知らせ」
# は原本ではないと思われるで、別途、医療費の領収書を保管しておく
# 必要があると思われます。
#
# ≪使い方≫ +++++++++++++++++++++++++++++++++++++++++++++++++++++++
